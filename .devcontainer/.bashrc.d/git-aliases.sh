...
function git_version_at_least() {
  [[ "$(printf '%s\n' "$1" "$git_version" | sort -V | head -n 1)" == "$1" ]]
}
...
function current_branch() {
  printf '%s\n' "'current_branch' is deprecated; use 'git_current_branch' instead." >&2
  git_current_branch "$@"
}
